...
po.map = function() {
  ...
  map.zoomBy = function(z, x0, l) {
    ...
    if (arguments.length < 2){
		return map.zoom(zoom + zoomFraction + z);
	}
    ...
    if (e.tile.proxyRefs) {
      for (var proxyKey in e.tile.proxyRefs) {
        ...
        if ((--proxyTile.proxyCount <= 0) && cache.unload(proxyKey)) {
          	if(proxyTile.element.parentNode)proxyTile.element.parentNode.removeChild(proxyTile.element);
        }
      }
  ...
  drag.map = function(x) {
    ...
    if (map) {
      container.removeEventListener("mousedown", mousedown, false);
		// (seanc | 3/21/2011)
		// check to see if this is svgweb, 
		// remove mousemove & mouseout from the root SVGweb element, not sure why were not removing window ones
		if(container.fake){
    ...
    if (map = x) {
      container = map.container();
		// (seanc | 3/21/2011)
		// check to see if this is svgweb, 
		// apply mousemove & mouseout to the root SVGweb element, because applying to window, never reaches map
		// left ones for window in there below, just because
		if(container.fake){
  ...
  function stylist(e) {
    ...
    for (i = 0; i < ne; ++i) {
      ...
      if (v = title) {
        ...
        if (v != null) o.appendChild(po.svg("title")).appendChild(document.createTextNode(v,true));
      }
    }
  }
