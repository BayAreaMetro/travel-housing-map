...
(function(mtc) {
	...
	mtc.travelTimeMap = function(selector) {
		...
		controller.init = function() {
			...
			var modeselektor = form.find("select[name=mode]");

			var state = {};
			state.direction = form.find("select[name=direction]").change(function() {
				state.direction = $(this).val();
				loadScenario();
			}).val() || "from";
			state.mode = modeselektor.change(function() {
				state.mode = $(this).val();
				layer.reshow();
			}).val() || "da";
