	...
	mtc.travelTimeMap = function(selector) {
		...
		*/
		...
		var style = po.stylist()
			.title(getTitle) // FIX ME: svgweb requires a second parameter (true) to be sent when creating a textNode. So just make sure polymaps is set up for that...
			.attr("title", getTitle)
...
/**
 * Special init function for svgweb use
 * TODO: possibly eliminate this?
**/
function beginMapCreation(){
		if(window.svgweb){
			var svg = document.createElementNS("http://www.w3.org/2000/svg", 'svg');
			svg.setAttribute("width","100%");
			svg.setAttribute("height","500px");
			
			var root = $("#travel-time")[0];
			
			var listener = function(evt) {
				
				svg.removeEventListener('SVGLoad', listener, false);
				svg = this; // important
				
				// check to see if svg is real or fake
				// if fake send the svg
				// if real remove svgweb created svg and let polymaps create
				if(svg.fake){
					finishMap(svg);
				}else{
					window.svgweb.removeChild(svg, root);
					finishMap(null);
				}
				
			};
			
			svg.addEventListener('SVGLoad', listener, false);
			// now append the SVG root to our document
			window.svgweb.appendChild(svg, root);
		}else{
			finishMap(null);
		}
}

/* remove self enclosed function */
function finishMap(svg){
	try {
	var _svgElm = svg;
	var container = $("#travel-time").htmapl(null,null,svg),
			page = $("#page"),
	...
	function setMapHeight(){
		try {
			...
			if(_svgElm){
				_svgElm.setAttribute("width",_mapWidth);
				_svgElm.setAttribute("height",_mapHeight);
			}
